...
export class GenAILiveClient {
  ...
  public async connect(config: LiveConnectConfig): Promise<boolean> {
    if (this._status === 'connected' || this._status === 'connecting') {
      console.log('[GenAILiveClient] Already connected or connecting.');
      return false;
    }
    ...
    // LOG CONFIG
    console.log('[GenAILiveClient] Connecting with config:', JSON.stringify(config, null, 2));
    ...
    console.log('[GenAILiveClient] Connected successfully.');
    return true;
  }

  public disconnect() {
    console.log('[GenAILiveClient] Disconnecting...');
    this.session?.close();
    this.session = undefined;
    this._status = 'disconnected';
  ...
  public send(parts: Part | Part[], turnComplete: boolean = true) {
    console.log('[GenAILiveClient] Sending parts:', parts, 'turnComplete:', turnComplete);
  ...
  public sendRealtimeInput(chunks: Array<{ mimeType: string; data: string }>) {
    console.log('[GenAILiveClient] Sending realtime input:', chunks);
    if (this._status !== 'connected' || !this.session) {
      this.emit('error', new ErrorEvent('Client is not connected'));
      return;
    }
  ...
  public sendToolResponse(toolResponse: LiveClientToolResponse) {
    console.log('[GenAILiveClient] Sending tool response:', toolResponse);
    if (this._status !== 'connected' || !this.session) {
      this.emit('error', new ErrorEvent('Client is not connected'));
      return;
    }
    ...
    console.log('[GenAILiveClient] Tool response sent.');
  }
